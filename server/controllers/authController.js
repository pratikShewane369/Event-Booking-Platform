...
exports.registerUser = async (req, res) => {
    ...
    try {
        ...
        await redisClient.setEx(`otp:account_verification:${email}`, OTP_TTL_SECONDS, otp);
...
exports.loginUser = async (req, res) => {
    ...
    if (!user.isVerified && user.role === 'user') {
        ...
        // Overwriting the key automatically replaces any old OTP — no separate delete needed
        await redisClient.setEx(`otp:account_verification:${email}`, OTP_TTL_SECONDS, otp);
        await sendOTPEmail(email, otp, 'account_verification');
...
exports.verifyOtp = async (req, res) => {
    ...
    const storedOtp = await redisClient.get(`otp:account_verification:${email}`);
    ...
    await redisClient.del(`otp:account_verification:${email}`); // Remove used OTP
